...
init(Parent) ->
    ...
    mnesia_schema:init(IgnoreFallback),
		%VRV
		ets:new(mnesia_dirty_locks,[named_table,public,set]),
...
do_update_op(Tid, Storage, {{Tab, K}, {RecName, Incr}, update_counter}) ->
    ...
    element(3, NewObj);
% VRV
do_update_op(Tid, Storage, {{Tab, Key}, Fun, dirty_update}) ->
	Result=
    try
      {New,Old}=dirty_update(Storage,Tab,Key,Fun),
      case New of
        undefined->
          commit_delete(?catch_val({Tab, commit_work}), Tid, Storage, Tab, Key, {Tab,Key}, undefined);
        _->
          commit_write(?catch_val({Tab, commit_work}), Tid, Storage,Tab, Key, New, undefined)
      end,
      {New,Old}
...
dirty_update(Storage, Tab, Key, Fun)->
  TrapExit=process_flag(trap_exit, true),
	{Queue,Old}=dirty_value_get(Storage,Tab,Key),
  Result=
    try
			try
				New=Fun(Old),
				dirty_dump_storage(Storage,Tab,Key,Queue,New),
				{ok,{New,Old}}
      catch _:FunError->
				mnesia_lib:error("~p",[FunError]),
				dirty_dump_storage(Storage,Tab,Key,Queue,Old),
				{error,{user_fun_error,FunError}}
			end
    catch
      _:DumpError->
        mnesia_lib:error("dump error ~p",[DumpError]),
        {error,DumpError}
    end,
...
dirty_value_get(Storage,Tab,Key)->
	case ets:update_counter(mnesia_dirty_locks,{Tab,Key},[{2,1},{3,2}],{{Tab,Key},0,0}) of
		[Queue,2]->
      case mnesia_lib:db_get(Storage,Tab,Key) of
		...
		[Queue,_Count]->
			ets:insert(mnesia_dirty_locks,{{{Tab,Key},Queue},self()}),
			{Queue,wait_queue()}
	end.

wait_queue()->
	receive
		{'$mnesia_dirty_lock',Value}->Value
	end.

dirty_dump_storage(Storage,Tab,Key,Queue,Value)->
	case dirty_check_queue({Tab,Key},Queue,1) of
		none->
      if
        Value=:=undefined ->mnesia_lib:db_erase(Storage, Tab, Key);
        true ->mnesia_lib:db_put(Storage, Tab, Value)
      end,
      case dirty_check_queue({Tab,Key},Queue,0) of
        none->ets:match_delete(mnesia_dirty_locks,{{Tab,Key},'_',0});
        PID->PID!{'$mnesia_dirty_lock',Value}
      end;
		PID->
      PID!{'$mnesia_dirty_lock',Value},
      ets:update_counter(mnesia_dirty_locks,{Tab,Key},{3,-1})
	end.

dirty_check_queue(Key,Queue,Count)->
	case ets:update_counter(mnesia_dirty_locks,Key,{3,-1}) of
		Count->none;
		_->dirty_next_queue({Key,Queue+1})
	end.

dirty_next_queue(Queue)->
  case ets:take(mnesia_dirty_locks,Queue) of
    [{_,PID}]->PID;
    []->
      timer:sleep(1),
      dirty_next_queue(Queue)
